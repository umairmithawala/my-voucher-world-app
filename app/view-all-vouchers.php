...
<?php
   ...
   ?>
<?php
   //here we are getting whats inside the type
   if (!empty($_GET['type'])) {
       $type = $_GET['type'];
   }
   else{
...
<html lang="en">
   ...
   <body>
      ...
      <?php
         if($type == "brand_vouchers" || $type == "trending_brand_vouchers" || $type == "less_price_vouchers"){
      ...
      <div class="row">
         ...
         <span style="font-size:30px; font-weight:400;">
            <?php
               if($type == 'brand_vouchers'){
                  echo "Brand Vouchers";
               }else if($type == 'dynamic_vouchers'){
                  echo "Dynamic Vouchers";
               }else if($type == "trending_brand_vouchers"){
                  echo "Trending Brand Vouchers";
               }else if($type == "trending_dynamic_vouchers"){
                  echo "Trending Dynamic Vouchers ";
               }else if($type == "less_price_vouchers	"){
                  echo "Less Price Vouches ";
               }else {
                  echo "Vouchers";
               }
      ...
      <div class="row">
         <?php 
            if($type == "brand_voucher"){
               $query2 = "SELECT * FROM `voucher` WHERE `is_deleted` = 0 AND `codes_available` = 1";
            }else if($type == "trending_brand_vouchers"){
               $query2 = "SELECT * FROM `voucher` WHERE `is_deleted` = 0 AND `codes_available` = 1 AND `rating` > 4";
            }else if($type == "less_price_vouchers"){
               $query2 = "SELECT * FROM `voucher` WHERE `is_deleted` = 0 AND `codes_available` = 1 AND `discount_price` < 100";
            }else{
               $query2 = "SELECT * FROM `voucher` WHERE `is_deleted` = 0 AND `codes_available` = 1";
            }
      ...
      <?php
         }else if($type == "dynamic_vouchers" || $type == "trending_dynamic_vouchers"){
      ...
      <br>
         ...
         <div class="row">
            <?php 
               if($type == "dynamic_vouchers"){
                  $query2 = "SELECT * FROM `variable_voucher` WHERE `is_deleted` = 0 AND `codes_available` = 1";
               }else if($type == "trending_dynamic_vouchers"){
                  $query2 = "SELECT * FROM `variable_voucher` WHERE `is_deleted` = 0 AND `codes_available` = 1 AND `rating` >";
               }else{
                  $query2 = "SELECT * FROM `variable_voucher` WHERE `is_deleted` = 0 AND `codes_available` = 1";
