...
if (!empty($_GET['id']) && !empty($_GET['type'])) {
    ...
    if($voucher_type == "normal" || $voucher_type == "dynamic"){
        ...
        $query1 = "SELECT `id` FROM `favorite` WHERE `user_id` = $user_id AND `voucher_id` = $voucher_id AND `voucher_type` = '$voucher_type'";
...
if($already_favorite == 0){
        $query = "INSERT INTO `favorite`(`id`, `user_id`, `voucher_id`,`voucher_type`,timestamp) VALUES (NULL,$user_id,$voucher_id,'$voucher_type',$timestamp)";
